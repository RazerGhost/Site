...
// doesn't have to carry every post/project's stripped body text.
export const GET: RequestHandler = () => {
	const entries = [
		...getAllDevlogEntries().map(({ slug, title, excerpt, searchText }) => ({
			slug,
			title,
			excerpt,
			searchText
		})),
		...getAllProjects().map(({ slug, name, description, searchText }) => ({
			slug,
			title: name,
			excerpt: description,
			searchText
