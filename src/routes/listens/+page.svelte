<script lang="ts">
	...
	const heatmapWeeks = $derived.by(() => {
		...
		for (let i = 0; i < startPad; i++) days.push({ date: '', plays: 0, level: -1 });
	...
	// --- Search ---
	let query = $state('');
	let searchResults = $state<{ track: string; artist: string; plays: number; spotifyUri: string | null }[]>([]);
	...
	function onSearchInput() {
		...
		searchTimeout = setTimeout(async () => {
			const res = await fetch(`/api/listening/search?q=${encodeURIComponent(q)}`);
	...
	let artistTracks = $state<{ track: string; plays: number; spotifyUri: string | null }[]>([]);
	...
	async function toggleArtist(artist: string) {
		...
		const res = await fetch(`/api/listening/artist?artist=${encodeURIComponent(artist)}`);
		const body = await res.json();
...
<main class="mx-auto max-w-6xl px-6 py-16">
	<div class="flex flex-wrap items-start justify-between gap-4">
		<div>
			<h1 class="text-3xl font-extrabold tracking-tight text-white" data-hero-reveal="0">Listens</h1>
	...
	{:else}
		...
		<div class="relative mt-6" use:reveal>
			<Search size={15} class="pointer-events-none absolute top-1/2 left-3 -translate-y-1/2 text-dim" aria-hidden="true" />
			<input
				type="search"
			...
			{#if searchResults.length}
				<ul class="absolute z-10 mt-1 max-h-72 w-full overflow-y-auto rounded-lg border border-border bg-surface shadow-[var(--shadow-card-hover)]">
						...
						<li>
							...
							>
								<span class="min-w-0 flex-1 truncate text-white"
									>{r.track} <span class="text-dim">— {r.artist}</span></span
								>
								<span class="shrink-0 text-xs text-dim">{r.plays} plays</span>
							</a>
						</li>
					{/each}
				</ul>
			...
			<div class="rounded-lg border border-border p-5 sm:p-6" use:reveal>
				...
				{#if hourlyByHour.some((n) => n > 0)}
					<div class="mt-5 border-t border-border pt-5">
						<p class="text-xs font-medium uppercase tracking-wide text-dim">Listening clock</p>
						<div class="mt-3 flex h-16 items-end gap-[3px]">
							{#each hourlyByHour as plays, hour}
								<div
									class="flex-1 rounded-sm bg-primary/70"
									style:height="{Math.max(4, (plays / maxHourly) * 100)}%"
								...
								<li>
									...
									>
										<div class="flex items-center justify-between text-sm">
											<span class="text-white hover:text-primary">{artist.artist}</span>
									...
									{#if expandedArtist === artist.artist}
										<ul class="mt-2 ml-3 flex flex-col gap-1.5 border-l border-border pl-3">
											{#each artistTracks as t}
												{@const href = trackHref(t.spotifyUri)}
												<li class="flex items-center justify-between text-xs">
													{#if href}
														<a {href} target="_blank" rel="noreferrer" class="link truncate text-gray hover:text-primary"
															>{t.track}</a
														>
													{:else}
														<span class="truncate text-gray">{t.track}</span>
													{/if}
													<span class="shrink-0 text-dim">{t.plays}</span>
												</li>
											{/each}
										</ul>
									{/if}
		...
		{#if data.onThisDay.length}
			<div class="mt-4 rounded-lg border border-border p-5 sm:p-6" use:reveal>
				<p class="text-xs font-medium uppercase tracking-wide text-dim">On this day</p>
				<ul class="mt-4 flex flex-col gap-3">
					{#each data.onThisDay as entry}
						{@const href = trackHref(entry.spotifyUri)}
						<li class="flex items-center justify-between gap-3 text-sm">
							<span class="w-14 shrink-0 text-dim">{entry.year}</span>
							<span class="min-w-0 flex-1 truncate text-white">
								{#if href}
									<a {href} target="_blank" rel="noreferrer" class="link hover:text-primary">{entry.track}</a>
								{:else}
									{entry.track}
								{/if}
							...
							{#each week as day}
								...
								{:else}
									<div
										...
										style:opacity={day.level > 0 ? 0.25 + day.level * 0.1875 : 1}
