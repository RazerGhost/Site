...
See the top-level README's [Persistent data](../README.md#persistent-data) section for the three SQLite files under `data/`. The critical thing the Dockerfile's `VOLUME ["/app/data"]` line does **not** do: persist anything across a Coolify redeploy. Coolify replaces the container from the image on every deploy, so an anonymous Docker volume is discarded along with it.
